...
class Node:
    ...
    def __init__(self,tags,dims,data=None,envs=None):
        ...
        if data is not None:
            self.data = tf.reshape(tf.cast(data,tf.float32),dims)
            self.data /= tf.reduce_max(tf.abs(self.data))
        else:
            self.data = tf.random_uniform(dims)
        if envs is not None:
            self.envs = []
            for i,j in zip(dims,envs):
                if j is None:
                    self.envs.append(tf.ones(i))
                else:
                    tmp = tf.cast(j,tf.float32)
                    tmp /= tf.reduce_max(tf.abs(tmp))
                    assert tmp.shape == (i,)
                    self.envs.append(tmp)
        else:
            self.envs = [tf.ones(i) for i in dims]
        self.dims = list(dims)
        self.tags = list(tags)
    ...
    def contract(T1,tags1,T2,tags2,tags_dict1=dict(),tags_dict2=dict()):
        ...
        #initiate the answer
        T = Node(tags,dims,tf.tensordot(TD1,TD2,[order1,order2]),envs)
        return T
    ...
    def svd(self,num,tag1,tag2,cut):
        ...
        tags2 = self.tags[num:] + [tag2]
        dims1 = dims1 + [cut]
        dims2 = dims2 + [cut]
        envs1 = self.envs[:num] + [env]
        envs2 = self.envs[num:] + [env]
